...
export function Onboarding() {
  ...
  return (
    <div className="rounded-2xl glass-card p-8 animate-fade-in">
      ...
      <div className="mt-8 flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50/60 px-4 py-3 text-xs text-slate-600">
        ...
        <span>
          {locale === 'sv' ? 'Bevakar ni myndigheter?' : 'Monitoring regulatory sources?'}
        </span>
        <a href="/dashboard?ref=compliance" className="font-semibold text-blue-600 hover:text-blue-700 transition">
          {locale === 'sv' ? 'Starta compliance-onboarding →' : 'Start compliance onboarding →'}
        </a>
      </div>
    </div>
  );
